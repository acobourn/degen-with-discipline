...
export function confirmPick({ evPct, modelLean = 0, bookCount, dataPoints = 0 }) {
  ...
  // confidence: base on EV (capped), nudged by agreeing model lean and data depth.
  const evComponent = Math.min(40, evPct * 600);     // 5% EV -> 30 pts
  const leanComponent = modelLean > 0 ? 12 : 0;
  const depthComponent = Math.min(18, dataPoints / 150);
  const bookComponent = Math.min(10, bookCount);
  const confidence = Math.round(50 + evComponent + leanComponent + depthComponent + bookComponent - 40);
